...
class MemoryOptimizedTrainer:
    ...
    def _enable_gradient_checkpointing(self):
        """Enable gradient checkpointing using model's built-in API"""
        if hasattr(self.model, 'gradient_checkpointing_enable'):
            self.model.gradient_checkpointing_enable()
            logger.info("✅ Gradient checkpointing enabled via model API")
            ...
            logger.info("✅ Gradient checkpointing enabled via wrapped model API")
        else:
            logger.warning("⚠️ Model does not support gradient_checkpointing_enable()")
