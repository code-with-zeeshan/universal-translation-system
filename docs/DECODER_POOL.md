...
- **Ports**: Coordinator default port is 5100. Decoder default is 8001.
- **File-based pool**: `config/decoder_pool.json` (overridable via `POOL_CONFIG_PATH`)
...
### Example configuration (`config/decoder_pool.json`)
```json
{
  "nodes": [
    {
...
Default file: `config/decoder_pool.json`. Customize via `POOL_CONFIG_PATH` env var.
...
```bash
bash scripts/setup_serving.sh --register --endpoint http://your-decoder:8001 --coordinator http://coordinator:5100
```

### Method 2: Coordinator API (v1 endpoints)
```bash
curl -X POST http://localhost:5100/api/v1/register \
...
```
2. Add node to `config/decoder_pool.json` and restart Coordinator.

## Removing or Disabling a Decoder
```bash
# API-based removal
curl -X DELETE http://localhost:5100/api/decoders/decoder-3 \
  -H "Authorization: Bearer YOUR_ADMIN_TOKEN"

# Set inactive
curl -X PATCH http://localhost:5100/api/decoders/decoder-3 \
  -H "Authorization: Bearer YOUR_ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"status": "inactive"}'
```
...
```bash
curl http://localhost:5100/api/decoders/decoder-3/health -H "Authorization: Bearer YOUR_ADMIN_TOKEN"
