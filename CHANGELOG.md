...
- **`download_evaluation` removed** from data pipeline stages — belongs to evaluation pipeline only (feature code kept, `uts eval --download` unchanged).
- **`direct_opus` stage removed** (fully redundant — auto-fallback in `download_training` already handles missing pairs via direct OPUS).
- **Quality stage descriptions** updated to reflect meaning-preserving mission: handling idioms, metaphors, and cultural equivalents (not word-for-word swaps).
